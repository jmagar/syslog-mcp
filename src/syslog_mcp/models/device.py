...
class LogActivitySummary(BaseModel):
    ...
    warning_count: conint(ge=0) = Field(
        default=0,
        description="Number of warning-level log entries"
    )
    
    critical_count: conint(ge=0) = Field(
        default=0,
    ...
    first_log_timestamp: Optional[datetime] = Field(
        None,
        description="Timestamp of oldest log entry in current dataset"
    )
    
    logs_per_hour: confloat(ge=0.0) = Field(
        default=0.0,
        description="Average logs per hour over recent period"
    )
    
    peak_logs_per_hour: confloat(ge=0.0) = Field(
        default=0.0,
    ...
    def hours_since_last_log(self) -> float:
        ...
        delta = now - self.last_log_timestamp.replace(tzinfo=timezone.utc)
...
class DeviceInfo(BaseModel):
    ...
    mac_addresses: List[constr(pattern=r"^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$")] = Field(
        default_factory=list,
    ...
    environment: Optional[constr(max_length=32)] = Field(
        None,
    ...
    log_activity: LogActivitySummary = Field(
        default_factory=LogActivitySummary,
    ...
    @classmethod
    def validate_ip_addresses(cls, v: Any) -> List[Union[IPv4Address, IPv6Address]]:
    ...
    def calculate_health_and_status(self) -> 'DeviceInfo':
        ...
        error_score = 1.0 - min(self.log_activity.error_rate, 1.0)
        ...
        self.health_score = sum(score * weight for score, weight in zip(health_factors, weights))
        
        # Determine status from health score and activity
        hours_since_last = self.log_activity.hours_since_last_log
    ...
    def _calculate_recency_score(self) -> float:
        ...
        hours_since = self.log_activity.hours_since_last_log
    ...
    def _generate_status_reason(self) -> str:
        ...
        if self.status == DeviceStatus.HEALTHY:
            ...
            reasons = []
            if self.log_activity.error_rate > 0.1:
                reasons.append(f"{self.log_activity.error_rate:.1%} error rate")
            if self.log_activity.hours_since_last_log > 6:
                reasons.append(f"{self.log_activity.hours_since_last_log:.1f}h since last log")
            ...
            reasons = []
            if self.log_activity.error_rate > 0.3:
                reasons.append(f"high error rate ({self.log_activity.error_rate:.1%})")
            if self.log_activity.hours_since_last_log > 12:
                reasons.append(f"{self.log_activity.hours_since_last_log:.1f}h inactive")
            ...
            hours = self.log_activity.hours_since_last_log
            if hours == float('inf'):
    ...
    def update_log_activity(self, log_summary: LogActivitySummary) -> None:
        ...
        self.calculate_health_and_status()
    
    def add_ip_address(self, ip: Union[str, IPv4Address, IPv6Address]) -> bool:
        """
    ...
    def get_primary_ip(self) -> Optional[Union[IPv4Address, IPv6Address]]:
    ...
    def is_active(self, hours_threshold: float = 24.0) -> bool:
        ...
        """
        return self.log_activity.hours_since_last_log <= hours_threshold
    ...
    def to_summary_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate": round(self.log_activity.error_rate, 3),
            "hours_since_last_log": round(self.log_activity.hours_since_last_log, 1),
...
class DeviceList(BaseModel):
    ...
    def get_top_critical(self, limit: int = 10) -> "DeviceList":
        ...
        return self.sort_by_criticality(descending=True).devices[:limit]