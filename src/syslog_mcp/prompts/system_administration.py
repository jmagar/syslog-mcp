...
"""
    ...
    def performance_investigation(
        ...
        focus_device: Optional[str] = Field(None, description="Specific device to focus investigation on")
    ) -> str:
        """
...
"""
    ...
    def daily_operations_review(
        review_date: Optional[str] = Field(None, description="Date to review (YYYY-MM-DD format, defaults to today)"),
...
**Alert Management:**
{f'- [ ] Alert rules are properly configured' if include_alerts else '- [ ] Alert system checks skipped'}
{f'- [ ] Alert notifications working properly' if include_alerts else ''}
{f'- [ ] No unresolved alerts requiring attention' if include_alerts else ''}
