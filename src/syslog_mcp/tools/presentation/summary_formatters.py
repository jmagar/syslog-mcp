...
def format_failed_auth_summary(analysis_data: Dict[str, Any]) -> str:
    ...
    # Create header
    title = f"Failed Authentication Summary"
    if device:
        title += f" - {device}"
    markdown = f"# {title}\n\n"
...
def format_suspicious_activity_summary(analysis_data: Dict[str, Any]) -> str:
    ...
    # Create header  
    title = f"Suspicious Activity Analysis"
    if device:
        title += f" - {device}"
    markdown = f"# {title}\n\n"
...
def format_auth_timeline_summary(analysis_data: Dict[str, Any]) -> str:
    ...
    markdown = f"# Authentication Timeline Analysis\n\n"
    markdown += f"**Analysis Period:** Last {hours} hours\n"
...
def format_ip_reputation_summary(analysis_data: Dict[str, Any]) -> str:
    """Format IP reputation analysis data into markdown summary."""
    
    ip_address = analysis_data.get("target_ip")
    hours = analysis_data.get("hours", 24)
    
    # Create header
    title = f"IP Reputation Analysis"
    if ip_address:
        title += f" - {ip_address}"
    markdown = f"# {title}\n\n"
    ...
    if ip_address:
        # Single IP analysis
        ip_data = analysis_data.get("ip_analysis", {})
        total_attempts = ip_data.get("total_attempts", 0)
        threat_score = ip_data.get("threat_score", 0)
        
        markdown += f"**IP Address:** {ip_address}\n"
        markdown += f"**Total Attempts:** {total_attempts:,}\n"
        ...
        markdown += f"**Threat Level:** {threat_emoji} {threat_level} (Score: {threat_score}/100)\n\n"
        
        # Geographic information
        geo_info = ip_data.get("geographic_info", {})
        if geo_info:
            markdown += "## Geographic Information\n\n"
            country = geo_info.get("country", "Unknown")
            city = geo_info.get("city", "Unknown")  
            organization = geo_info.get("organization", "Unknown")
            markdown += f"- **Country:** {country}\n"
            markdown += f"- **City:** {city}\n" 
            markdown += f"- **Organization:** {organization}\n\n"
        
        # Attack patterns
        attack_patterns = ip_data.get("attack_patterns", {})
        if attack_patterns:
            markdown += "## Attack Patterns\n\n"
            primary_pattern = attack_patterns.get("primary_pattern", "UNKNOWN")
            markdown += f"**Primary Pattern:** {primary_pattern}\n"
            
            targeted_services = attack_patterns.get("targeted_services", [])
        ...
        analyzed_ips = analysis_data.get("analyzed_ips", [])
        total_ips = len(analyzed_ips)
        
        markdown += f"**IPs Analyzed:** {total_ips}\n\n"
        
        if analyzed_ips:
            markdown += "## Top Threat IPs\n\n"
            
            # Sort by threat score
            sorted_ips = sorted(analyzed_ips, key=lambda x: x.get("threat_score", 0), reverse=True)
            ...
            for i, ip_data in enumerate(sorted_ips[:10], 1):
                ...
                markdown += f"{i}. **{ip}** ({country}) - {attempts:,} attempts, {threat_emoji} {threat_level} ({threat_score}/100)\n"
            
            markdown += "\n"
            
            # Geographic distribution
            geo_distribution = analysis_data.get("geographic_distribution", [])
            if geo_distribution:
                markdown += "## Geographic Distribution\n\n"
                for geo in geo_distribution[:10]:
                    country = geo.get("country", "Unknown")
                    attempts = geo.get("attempts", 0)
                    ip_count = geo.get("ip_count", 0)
                    markdown += f"- **{country}:** {attempts:,} attempts from {ip_count} IPs\n"
                markdown += "\n"
...
def format_error_analysis_summary(analysis_data: Dict[str, Any]) -> str:
    ...
    if error_trends:
        ...
        if trend == "INCREASING":
            ...
            markdown += f"➡️ **Error rate is stable** - No significant trend detected\n"
        markdown += "\n"
...
def format_saved_searches_summary(searches_data: Dict[str, Any]) -> str:
    """Format saved searches data into readable summary."""
    
    searches = searches_data.get("searches", {})
    
    markdown = "# Saved Searches\n\n"
    
    if not searches:
        markdown += "📝 **No saved searches found.**\n\n"
        markdown += "Use `add_saved_search` to save your frequent searches for quick access.\n"
        return markdown
    
    markdown += f"**Total Saved Searches:** {len(searches)}\n\n"
    
    # Sort by usage count and last used
    sorted_searches = sorted(
        searches.items(),
        key=lambda x: (x[1].get("usage_count", 0), x[1].get("last_used") or ""),
        reverse=True
    )
    
    for name, search_data in sorted_searches:
        markdown += f"## {name}\n"
        markdown += f"**Query:** `{search_data.get('query', 'N/A')}`\n"
        markdown += f"**Type:** {search_data.get('search_type', 'general')}\n"
        markdown += f"**Usage Count:** {search_data.get('usage_count', 0)}\n"
        
        if search_data.get('description'):
            markdown += f"**Description:** {search_data['description']}\n"
        
        if search_data.get('last_used'):
            markdown += f"**Last Used:** {search_data['last_used']}\n"
        
        markdown += "\n"
    
    markdown += f"---\n*Searches loaded at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*\n"
    return markdown
