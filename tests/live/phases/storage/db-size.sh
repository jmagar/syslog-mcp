...
jq -cn --argjson fixture_bytes "$fixture_bytes" --argjson logical_size_bytes "$(jq -r .logical_size_bytes "$status")" --argjson protected_errors "$count" \
  '{schema:"cortex-live-db-size-pressure-v1",disposition:"pass",fixture_bytes:$fixture_bytes,recovered_below_bytes:8388608,logical_size_bytes:$logical_size_bytes,error_floor_per_source_cap:10,remaining_errors:$protected_errors,observed_excess_errors_deleted:($protected_errors<715),observed_floor_minimum_preserved:($protected_errors>=10)}' \
