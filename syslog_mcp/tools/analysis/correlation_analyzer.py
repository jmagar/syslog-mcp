...
def analyze_search_correlate_data(
    ...
    # Timeline insights
    peak_activity = safe_int(timeline_stats["peak_activity"]) if timeline_stats else 0
    total_windows = safe_int(timeline_stats["total_windows"]) if timeline_stats else 0
    if peak_activity > 0:
        avg_activity = total_events / total_windows if total_windows > 0 else 0
        if peak_activity > avg_activity * 3:
            insights.append(f"Significant activity spike detected at {timeline_stats['peak_time']} with {peak_activity} events")
