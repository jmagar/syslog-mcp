...
def analyze_authentication_timeline_data(
    ...
    if "auth_timeline" in aggs:
        for bucket in aggs["auth_timeline"]["buckets"]:
            ...
            # Extract success/failure counts from filter aggregations
            successful_attempts = bucket.get("successful_auths", {}).get("doc_count", 0)
            failed_attempts = bucket.get("failed_auths", {}).get("doc_count", 0)
...
def _identify_peak_authentication_periods(timeline_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def _analyze_authentication_trends(timeline_data: list[dict[str, Any]]) -> dict[str, Any]:
...
def _identify_activity_patterns(activity_data: list[dict[str, Any]], hours: int) -> list[dict[str, Any]]:
...
def _detect_activity_anomalies(activity_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def _calculate_activity_consistency(activity_data: list[dict[str, Any]]) -> dict[str, Any]:
...
def _extract_temporal_features(timeline_data: list[dict[str, Any]]) -> dict[str, Any]:
...
def _identify_cyclical_patterns(timeline_data: list[dict[str, Any]], pattern_type: str) -> list[dict[str, Any]]:
...
def _detect_daily_pattern(timeline_data: list[dict[str, Any]]) -> dict[str, Any] | None:
...
def _detect_temporal_anomalies(timeline_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def _estimate_interval(timeline_sample: list[dict[str, Any]]) -> float:
...
def _calculate_pattern_strength(patterns: list[dict[str, Any]]) -> dict[str, Any]:
...
def _calculate_trend(values: list[float]) -> dict[str, Any]:
    ...
    if abs(slope) < 0.1:
        ...
        strength = min(abs(slope) * 10, 100.0)  # Scale to 0-100
    else:
        direction = "DECREASING"
        strength = min(abs(slope) * 10, 100.0)
