...
def analyze_error_message_patterns(error_list: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    pattern_stats.sort(key=lambda x: safe_int(x["count"]), reverse=True)

    # Identify recurring issues
    recurring_issues = [p for p in pattern_stats if safe_int(p["count"]) > 2]

    # Calculate pattern diversity
    total_errors = sum(safe_int(p["count"]) for p in pattern_stats)
...
def _analyze_detailed_error_patterns(
    ...
    sample_errors: dict[str, list[Any]]
) -> dict[str, Any]:
