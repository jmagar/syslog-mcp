...
def _assess_component_health(program: str, recent_errors: list[dict[str, Any]]) -> str:
...
def _calculate_activity_variance(timeline: list[dict[str, Any]]) -> float:
    """Calculate variance in activity levels."""

    if not timeline:
        return 0.0

    counts = [int(period.get("log_count", 0)) for period in timeline]
