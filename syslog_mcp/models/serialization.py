...
class ModelDeserializer:
    ...
    def _recover_from_validation_error(
        ...
        for error_dict in error.errors():
            field_path: tuple[Any, ...] = error_dict.get('loc', ())
            ...
            if field_path:
                ...
                if error_type == 'missing':
                    if field_name == 'timestamp' and model_class == LogEntry:
                        recovered_data['timestamp'] = datetime.now(UTC)
                ...
                # Handle type conversion errors
                    ...
                    if isinstance(recovered_data.get(field_name), str):
                        try:
                            ...
                            recovered_data[field_name] = datetime.now(UTC)
    ...
    **kwargs: Any
    ...
    if format == "json":
        if isinstance(data, str):
            ...
            raise ValueError("JSON format requires string data")
    elif format == "dict":
        if isinstance(data, dict):
            return deserializer.from_dict(model_class, data, **kwargs)
        else:
            raise ValueError("Dict format requires dictionary data")
    elif format == "elasticsearch":
        if isinstance(data, dict):
            return deserializer.from_elasticsearch_doc(model_class, data, **kwargs)
        else:
            raise ValueError("Elasticsearch format requires dictionary data")
    else:
        raise ValueError(f"Unsupported format: {format}")
